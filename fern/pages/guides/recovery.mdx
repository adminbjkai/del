---
title: Recovery
subtitle: Restoring DEL's database, nginx site, and venv
...
To recover `del-web` itself, see [Operations](/guides/operations) for restart, and the venv/DB
...
- **Inside `nginx_rm_site`** — the helper copies each site file to
  `/apps/del/backups/` first, removes them, runs `nginx -t`, and copies them back if
  the test fails. Self-contained; does not depend on the `backups` table at all, and
  has always worked.
- **Job-level restore** — when an `nginx_rm_site`, `nginx_test_reload`,
  ...
  audits the real per-backup outcome as `job_restore_attempted`. Volume archives are
  deliberately skipped and reported as manual-restore.
...
<Callout intent="warning">
  **If you are looking at a job from before 2026-08-24:** nothing ever wrote the
  ...
  recording `job_restore_attempted`. Those jobs were not rolled back, whatever the
  audit log says.
</Callout>
...
- **Helper rejects a valid-looking operation** — check the **deployed** policy at
  ...
  the deployed one) for the approved/protected root lists, `protected_units` and the
  never-delete list; then `/apps/del/logs/helper-audit.log` for the exact rejection
  reason — the helper logs every request including denials.
- **Helper is running stale code after an edit** — `del-helper.service` executes
  ...
  Re-run `/apps/del/scripts/install.sh` and then restart; a bare
  `systemctl restart` re-runs the old deployed copy. Confirm with
  `sudo diff /apps/del/helper/del_helper.py /usr/local/lib/del-helper/del_helper.py`.
