...
Chrome: primary `.btn` is `min-height: 2.35rem`; `.panel` radius is 12px (slightly rounder than the old 8px). Compact `.btn-sm` in the How-to action group is still ~2rem.
...
<Steps>
  ...
  <Step title="Resources">
    [Resources → Containers](https://del.bjk.ai/resources/container), then the other tabs. Owner chips are current apps. **Shared** = more than one current app — do not delete with a single app. Unassigned chips go to Orphans.
...
<AccordionGroup>
  ...
  <Accordion title="Unassigned / orphan">
    No probable current owner. Listed on Orphans. Not “Docker volume unused” and not “safe to delete here.”
  </Accordion>
