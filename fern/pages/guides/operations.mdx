...
runtime files out of Git, review incoming commits, and never overwrite a dirty
worktree.
...
The dashboard's scan strip reflects this live: once you click **Run scan now** the
button disables itself, a small bar and an elapsed-time counter appear ("scanning…
Ns"), and the page polls `GET /scan/status` every 3 seconds. When the scan finishes
the page reloads once on its own to show the refreshed stats and tables — you don't
need to refresh by hand or watch for it. The same dashboard has a **Needs
attention** panel (up to 10 entries) listing applications with uncertain
correlation, each linking straight to the app that needs a look.
