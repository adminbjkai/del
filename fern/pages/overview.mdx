...
When you want an application gone, DEL builds a **staged, dry-run-first removal
plan** and executes it through a separate privileged helper. Every deletion is
previewed before it runs, optionally backed up before it happens (choose a backup
mode — the default is *None*), and validated after. The one truly irreversible step
— deleting a named volume — is gated behind a typed confirmation phrase. DEL is
careful by construction: it cannot remove itself, it refuses shared resources unless
you approve them, and anything it is unsure about is preserved rather than deleted.
