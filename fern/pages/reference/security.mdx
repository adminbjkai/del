...
- The unauthenticated routes are exactly: `/login`, `/healthz`, `/favicon.ico`
  ...
  `/static/app.js`, `/static/theme-init.js`, `/static/favicon.svg`. Every other route requires a
...
- **Response headers**, all set in `config/nginx-del.bjk.ai.conf` with `always`:
  ...
  JS/CSS implementation (the vendored AG Grid bundle has been removed), there
  are no CDN scripts or web fonts, and app icons are proxied through
