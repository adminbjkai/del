...
`/` still requires DEL session login. The inventory export at `/miscwork.html`
remains basic-auth protected separately.
...
- `del-docs.service`: `NoNewPrivileges=true`, runs as `bjkai`; docs are open
  (no basic auth). It only serves static/rendered documentation content.
