...
- The unauthenticated routes are exactly: `/login`, `/healthz`, `/favicon.ico`
  ...
  `favicon.svg`, `assistant.css`, `assistant.js`, and `/static/vendor/` AG Grid).
  Every other route requires a
  session — including `/app-icon/{domain}` (the gallery's server-side favicon
  proxy) and `/palette.json` (the command-palette data feed). Unauthenticated
...
- Nothing writes a password to disk. `del-admin` takes it from a `getpass` prompt
  ...
  `/apps/del/config/admin-initial-password.txt` this page used to describe has been
  deleted and is not regenerated.
...
- Login is rate-limited to 5 attempts per 60 seconds per IP — a plain in-memory
  sliding window. There is no escalating backoff and no persistence: a `del-web`
...
- `/usr/local/lib/del-helper/` and `/etc/del/helper-policy.json`: `root:root`,
  not writable by `bjkai` — see below for why that is load-bearing.
...
`scripts/install.sh` with `install -o root -g root`. The repo copies
(`helper/*.py`, `config/helper-policy.json`) are the source you edit; the deployed
copies are what root executes and reads.

<Callout intent="warning">
  This split is the point of the boundary, not a packaging detail. `/apps/del` is
  writable by `bjkai`, the account `del-web` runs as — so pointing `ExecStart` at
  the working tree would let a compromised web tier rewrite the code root is about
  to run, and rewrite the allowlist that bounds it. After editing `helper/` or
  `config/helper-policy.json`, re-run `scripts/install.sh` before restarting
  `del-helper`; a plain restart re-runs the old deployed copy.
</Callout>
...
confirmation phrase at execution time (see
...
<Callout intent="info">
  The helper has no concept of a plan. It reads `op`, `args` and `dry_run`; it also
  records `plan_id`, `step_id`, `job_id` and `requested_by` in its audit line, but
  it cannot verify them — it never sees the `plans` table. What bounds a
  compromised web tier is the fixed op allowlist plus the helper's independent
  argument validation against `/etc/del/helper-policy.json`. Read every "must
  appear in the approved plan" phrasing in these docs as a *planner* constraint,
  not a helper one.
</Callout>
...
- **Audit correlation** — every helper request logs `plan_id`, `step_id`, `job_id`
  ...
  attributable to a DEL user and job rather than matched to `audit_log` by
  timestamp.
...
separately only emits paths derived from an approved plan, but as above, the helper
cannot check that.)

<Callout intent="info">
  `/home/bjkai` stays on the approved list despite being a home directory: the
  `code-server` unit bind-mounts it, so DEL needs to be able to clean up
  projects under it the same way it does under `/apps`. This is a known
  trade-off, not an oversight — nothing else in `/home` is in scope.
</Callout>
...
the realpath computed at validation time — closing the window between
validation and deletion during which an attacker with filesystem control
could swap a symlink to point somewhere else.
...
HTTP basic auth** (open documentation). Those paths bypass the app entirely and
are proxied straight to the docs dev server. Docs serve only static/rendered
content — no DEL database, helper socket, or session cookies. The **app UI** at
`/` still requires DEL session login.

The inventory export at `/miscwork.html` (and its `/inventory` alias) is the
**only** location in the vhost carrying an `auth_basic` directive. It uses
`/etc/nginx/.del-docs-htpasswd`; despite that filename, `/docs` does not.

`install.sh` installs neither `del-docs.service` nor the htpasswd file — see
...
- **Cockpit** (the systemd/services web UI for non-Docker apps) is bound to
  ...
  (similar pattern to DEL's inventory export at `/miscwork.html`).
...
| Compromised web tier → rewrite what root runs | Helper code and policy deployed `root:root` outside `/apps/del`; `protected_units` refuses to stop `del-helper` itself |
...
- **Response headers**, all set in `config/nginx-del.bjk.ai.conf` with `always`:
  ...
  JS/CSS table engine (AG Grid Community 32.3.3 vendored at `/static/vendor/`,
  vanilla fallback, no CDN), there are no CDN scripts or web fonts, and app icons are proxied through
  `/app-icon/` rather than loaded from a third-party origin, so no external
  asset origin is needed. `script-src 'self'` with no `'unsafe-inline'` is also
  why the pre-paint dark/light theme switch is its own external file,
  `/static/theme-init.js`, rather than an inline `<script>` in `base.html`. Note
  that nginx's `add_header` is not inherited into a `location` block that defines
  its own; none in this vhost do, but adding one means repeating these directives.
