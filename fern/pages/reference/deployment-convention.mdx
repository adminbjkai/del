...
for the underlying six-stage engine (`backup`, `quiesce`, `remove_runtime`,
`remove_host`, `remove_files`, `validate`).

Two things to set deliberately each time, because neither is the default:

- **Choose a backup mode.** The plan builder defaults to *None*, which takes no
  backup and leaves the automatic in-job rollback with nothing to restore from. Use
  *Config* or *Full* for anything you might want back.
- **Rescan first.** The plan builder scopes to the latest *completed* scan and
  refuses outright if the app has no resources there, rather than emitting an empty
  plan that would report success having removed nothing.
