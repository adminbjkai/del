...
Operator path: [scan → apps → resources → orphans → Ask → plan](/guides/sop).

```mermaid
flowchart LR
  S[Scan] --> I[SQLite inventory]
  I --> UI[Apps / Resources / Orphans]
  UI --> Ask[Ask rail]
  I --> Ask
  UI --> P[Plan builder]
  P --> D[Dry-run job]
  D --> L[LIVE job]
  L --> H[del-helper root socket]
```
...
| systemd_stop / systemd_disable / systemd_rm_unit | unit | name must match `^[A-Za-z0-9@_.-]+\.(service\|timer)$` and must not match `protected_units`; rm restricted to /etc/systemd/system + daemon-reload; `systemd_rm_unit` re-resolves the unit file's realpath (`recheck_realpath`) immediately before the delete call, closing the TOCTOU window between validation and removal |
...
│   │                      003_assistant.sql (assistant conversations + messages)
...
│   ├── scanner.py         run_scan() orchestration + scan_state() (in-process lock state, for polling)
...
`_glossary.html`, `_assistant_dock.html`). Authenticated `base.html` loads `app.css`,
AG Grid Community 32.3.3 from `/static/vendor/`, `assistant.css` / `assistant.js`,
`theme-init.js`, `app.js`, and `<meta name="csrf-token">`. `login.html` loads only
`app.css`, `theme-init.js`, and the favicon. Those files are served unauthenticated
under `/static/*` (CSP `script-src 'self'`, no CDN).
...
- `DEL.assistant` (`assistant.js` on all authenticated pages) — `/assistant` plus the Ask tab of the Help|Ask rail.
- The table engine (`enhanceTable` on `table[data-enhanced]`) — AG Grid Community 32.3.3 vendored at `/static/vendor/` with a `.table-toolbar` (quick filter, Clear all filters, status); vanilla fallback if the script fails to load.
...
- audit_log(id, ts, user_id, action, subject, details_json)  — no secrets ever.
  ...
  policy, not an index.
- settings(key, value)
- assistant_conversations(id, user_id, scope, target, title, created_at, updated_at)
  and assistant_messages(id, conversation_id, role, content, prompt_id, usage_json,
  context_truncated, created_at) — migration `003_assistant.sql`; the inventory
  context sent to the model is rebuilt per turn and never stored.
...
(browsers request it unprompted; it used to 404 on every page load), and the
`/static/*` assets (`app.css`, `app.js`, `theme-init.js`, `favicon.svg`, `assistant.css`, `assistant.js`, and vendored AG Grid under `/static/vendor/`)
send `Cache-Control: public, max-age=300, must-revalidate`.

`POST /scan` starts a scan on a background thread and redirects immediately
with `flash=Scan+started`; `GET /scan/status` returns the scanner's current
`scan_state()` plus the most recent `scans` row as JSON, for the settings page
to poll. `/apps/{slug}`, `/apps/{slug}/plan` and `/plans/{id}` return 404 for
an unknown slug/id; `/plans/{id}/execute` returns 404 for an unknown plan id
and 409 for any other plan integrity failure; `/jobs/{id}/status` returns 404
JSON for an unknown job and otherwise includes `progress {done, total, pct}`
and `current_step`.

`POST /apps/{slug}/remove` is the one-click "Remove app now" path behind the
app detail page's button: it builds a complete-removal plan (every named
volume approved, `remove_images=exclusive`, bind data/repo/networks all on,
backup=none) through the same `planner.build_plan`/`persist_plan` used by the
step-by-step builder — so the same safe-delete checks and shared/`probable`
preservation apply — then runs it live immediately, supplying the typed
volume-deletion confirmation phrase internally rather than prompting for it.
404 for an unknown slug, 403 if the app is `protected`. The step-by-step
`/apps/{slug}/plan` builder, with its separate dry-run and live-execute
steps, still exists alongside it.
...
A compose project whose name matches an existing app is merged into it only
when the compose file's own top-level project directory also belongs to that
...
placeholder app if needed) at confidence 55 / `possible` / `shared`, with
evidence naming both the foreign owner and the same-named app it did not
merge into, instead of being silently absorbed at full confidence.
...
call it. Separately, building a `compose_down` step also checks whether
another app's compose project would resolve to the same Docker Compose
project label under a different path, and if so appends a plan warning naming
both apps and paths — the label sweep is host-wide.
...
- Unauthenticated routes are exactly `/login`, `/healthz`, `/favicon.ico` and
  `/static/*` (`app.css`, `app.js`, `theme-init.js`, `favicon.svg`, `assistant.css`, `assistant.js`, vendored AG Grid).
