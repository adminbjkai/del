...
- **del-docs.service** — serves this documentation site (Fern, `fern/` sources) on
  ...
  behind HTTP basic auth, independent of the app's own session auth. It carries no
  privileged access — it only serves static/rendered docs content.
...
flowchart LR
    ...
    EV --> SC{"Confidence"}
    SC --> CF["confirmed / high<br/>→ eligible"]
    SC --> PB["probable<br/>→ needs approval"]
    SC --> PS["possible / shared<br/>→ blocked"]
```
...
naming the container and noting "(host network)". Docker networks are
...
disabled copies) that port-matching alone would miss. Name similarity alone =
possible, never auto-removable. Only confirmed/high/manual associations are
eligible for removal; probable requires explicit user approval per-resource;
possible is always blocked until manually confirmed.
...
A removal job runs a signed, approved plan through nine ordered stages. Any
safety-validation failure halts the job before further deletions; nginx/systemd
steps auto-restore from their backups, and the whole job is resumable from the
failed step.

```mermaid
flowchart TD
    A["1 · Analyze<br/>re-check live state"] --> PV["2 · Preview<br/>(dry-run by default)"]
    PV --> BK["3 · Backup<br/>file/volume/tar before any mutation"]
    BK --> QU["4 · Quiesce<br/>stop containers/units/processes"]
    QU --> RR["5 · Remove runtime<br/>compose down · container/image/volume/network rm"]
    RR --> RH["6 · Remove host<br/>systemd · cron · nginx site"]
    RH --> RF["7 · Remove files<br/>path_delete (canonicalized, protected-root check)"]
    RF --> VA["8 · Validate<br/>targets gone · nginx -t ok"]
    VA --> RP["9 · Report<br/>status · reclaimed bytes · audit"]

    QU -->|"step fails"| HALT{"Halt job"}
    RR -->|"step fails"| HALT
    RH -->|"nginx/systemd fails"| RB["Auto-restore from<br/>stage-3 backup"]
    RF -->|"step fails"| HALT
    VA -->|"check fails"| HALT
    RB --> HALT
    HALT -.->|"operator fixes cause, retries"| RESUME["Resume at failed step<br/>(completed steps not re-run)"]
    RESUME --> RR
```

The volume-deletion gate sits on the `volume_rm` step in stage 5: it runs live only
with the plan option, the per-volume checkbox, and the typed `DELETE VOLUMES` phrase
all present. See [Removing an Application](/guides/removing-an-application) for the
