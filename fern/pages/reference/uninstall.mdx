...
4. **Remove the inventory export's basic-auth password file.** Despite the name, it
   protects only `/miscwork.html` (and its `/inventory` alias); `/docs` has no
   `auth_basic` directive at all:
   ```bash
   sudo rm -f /etc/nginx/.del-docs-htpasswd
   ```
   (Any leftover plaintext under `/apps/del/config/` is removed with the tree in step 6.)
