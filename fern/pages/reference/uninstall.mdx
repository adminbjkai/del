...
4. **Remove the inventory basic-auth password file** (if present; used only for
   `/miscwork.html` / `/inventory`, not for docs):
   ```bash
   sudo rm -f /etc/nginx/.del-docs-htpasswd
   ```
   (Any leftover plaintext under `/apps/del/config/` is removed with the tree in step 5.)
