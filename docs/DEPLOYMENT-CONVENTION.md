...
3. **Enable + start:**
   ...
   ```
4. **View/control it day-to-day via Cockpit** (`https://cockpit.bjk.ai`, the
   systemd/services web UI) — status, logs, start/stop/restart — instead of
   SSHing in for routine checks. Cockpit's own socket is bound to
   `127.0.0.1:9091` only (not reachable directly from the internet); the
   `cockpit.bjk.ai` nginx vhost is the sole path in, and it sits behind HTTP
   basic auth like the DEL docs site does.
5. **nginx vhost** from the template in §6, `proxy_pass` to
   `http://127.0.0.1:<host_port>`.
...
/etc/nginx/sites-enabled/*;` has **no filename filter** (it is not `*.conf`,
deliberately — this server has ~150 sites named `<name>.bjk.ai` with no
extension), which means *any* stray file dropped in `sites-enabled` — a
`.bak`, a `.orig`, an editor swap file, a byhand copy — is parsed as a vhost on
the next reload and can break it. When editing a live vhost, always edit (and
back up) the `sites-available` copy and leave the `sites-enabled` symlink
untouched; if you ever need to save an in-place copy of a config, put it
outside `sites-enabled` (e.g. `/etc/nginx/config-backups/`, or let DEL's
`file_backup`/`nginx_rm_site` do it under `/apps/del/backups`), never inside
`sites-enabled` itself.
...
any time before or after the nginx vhost exists. The script's IONOS API key and
zone ID live in `/etc/isbd.env` (mode `640`, owner `root:bjkai`, sourced by the
script) — not in the script itself — so the credential isn't exposed in
plaintext to anyone who can merely read `/usr/local/bin/isbd`.
...
volumes:
  - <name>_data                   # Docker named volume(s) this app owns
cron:
